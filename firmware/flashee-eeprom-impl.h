...
class ExternalSPIFlashDevice : public FlashDevice {
public:
    /**
     * Constructor
     */
     ExternalSPIFlashDevice(ExtSpiFlash *extSpiFlash) {
         _extSpiFlash=extSpiFlash;
     }
     ...
     */
    virtual page_count_t pageCount() const {
        return _extSpiFlash->pageCount();
    }

    virtual bool erasePage(flash_addr_t address) {
        bool success = false;
        if (address < pageAddress(pageCount()) && (address % pageSize()) == 0) {
            _extSpiFlash->EraseSector(address);
            success = true;
        }
        return success;
    }
    ...
    virtual bool writePage(const void* data, flash_addr_t address, page_size_t length) {
        // TODO: SPI interface shouldn't need mutable data buffer to write?
        _extSpiFlash->WriteBuffer(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(data)), address, length);
        return true;
    }

    virtual bool readPage(void* data, flash_addr_t address, page_size_t length) const {
        _extSpiFlash->ReadBuffer((uint8_t*) data, address, length);
        return true;
    }
